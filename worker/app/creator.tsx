...
export function Creator() {
  ...
  async function uploadVideo(files: FileList | null) {
    const file = files?.[0];
    if (!file) return;
    if (file.type !== 'video/mp4') return setMessage('Please select an MP4 video.');
    setMessage('Uploading your movement reference securely…'); setUploadPercent(0);
    try {
      const init = await fetch('/api/uploads/video/init', { method: 'POST' });
      const { pathname } = await init.json();
      if (!init.ok || typeof pathname !== 'string') throw new Error('Unable to prepare video upload.');
      const blob = await upload(pathname, file, {
        access: 'private', handleUploadUrl: '/api/uploads/video',
        onUploadProgress: ({ percentage }) => setUploadPercent(Math.round(percentage)),
      });
      setReferenceVideos((current) => [...current, blob.pathname]); setMessage('Movement reference saved privately.');
  ...
  return <section className="creator">
    ...
    <div className="photo-upload"><label>Optional: add a movement reference video<input type="file" accept="video/mp4" onChange={(event) => void uploadVideo(event.target.files)} /></label><p>Upload one MP4 up to 200 MB from your phone to guide natural movement and animation. {uploadPercent !== null ? `Uploading ${uploadPercent}%…` : referenceVideos.length ? 'Movement reference saved.' : ''}</p></div>
