...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ jobs: await listCompletedJobs(userId) });
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const job = { id, customerId: userId, type: 'video' as const, status: 'submitted' as const, prompt, createdAt: new Date().toISOString(), operation };
    await writeCurrentJob(job);
    return NextResponse.json({ job: { ...job, operation: undefined }, model }, { status: 202 });
  } catch (error) {
    const job = { id, customerId: userId, type: 'video' as const, status: 'failed' as const, prompt, createdAt: new Date().toISOString(), error: error instanceof Error ? error.message : 'Video generation request failed' };
    await writeCurrentJob(job);
