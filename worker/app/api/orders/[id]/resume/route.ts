...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Resume means resume the PREVIEW, not merely assembly. The direct runner is
  // idempotent: it skips completed scenes and restarts at the first incomplete one.
  after(async () => {
    try {
      ...
      console.error('[DirectPreview] resume generation failed', { orderId: id, error });
    }
  });
