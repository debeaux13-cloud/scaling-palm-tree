...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({
    ...
    final: order.tier ? { pages: order.finalStorybook?.pageCount ?? tierFor(order)?.storybookPages, usesExactSceneArtwork: true, usesExactSceneNarration: true, customerFacingSceneLabels: false, status: order.finalStorybook?.status } : null,
