...
async function generateScene(orderId: string, sceneNumber: number, fulfillment = false) {
  ...
  if (scene.generation.attempts >= MAX_SCENE_ATTEMPTS) throw new Error(`Scene ${sceneNumber} reached ${MAX_SCENE_ATTEMPTS} attempts; manual review required.`);
  let claimed = false;
  await mutateOrder(orderId, (fresh) => { const current = fresh.scenes[sceneNumber - 1]; if (!current) return; if (fulfillment && fresh.purchase.status !== 'paid') return; if ((current.status === 'completed' && current.videoPathname) || current.status === 'submitted' || current.generation.attempts >= MAX_SCENE_ATTEMPTS) return; current.status = 'submitted'; current.generation.attempts += 1; delete current.error; fresh.status = fulfillment ? 'fulfillment-in-progress' : 'preview-in-progress'; claimed = true; });
  if (!claimed) return;
...
export async function runDirectFulfillment(orderId: string) {
  ...
  console.info('[DirectMovie] PR82 paid fulfillment', { orderId, missingPaidScenes: sceneNumbers.length, sceneNumbers });
