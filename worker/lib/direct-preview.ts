...
async function generateScene(orderId: string, sceneNumber: number) {
  ...
  await mutateOrder(orderId, (fresh) => {
    ...
    current.status = 'submitted'; current.generation.attempts += 1; delete current.error; fresh.status = 'preview-in-progress';
  });
  try {
    ...
    await mutateOrder(orderId, (fresh) => {
      const current = fresh.scenes[sceneNumber - 1]; if (!current) return;
      current.videoPathname = pathname; current.status = 'completed'; delete current.error;
    });
