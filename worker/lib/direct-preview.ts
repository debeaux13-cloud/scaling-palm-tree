...
import { mutateOrder, previewSceneCount, readOrder, writeOrder } from './orders';
...
async function generateScene(orderId: string, sceneNumber: number) {
  ...
  await mutateOrder(orderId, (fresh) => {
    const current = fresh.scenes[sceneNumber - 1];
    if (!current) return;
    current.status = 'submitted';
    current.generation.attempts += 1;
    fresh.status = 'preview-in-progress';
  });
  ...
  if (!video) throw new Error(`Wan returned no video for scene ${sceneNumber}`);
  const pathname = `studio/orders/${orderId}/scenes/${sceneNumber}/movie.mp4`;
  const body = new Blob([video.uint8Array as Uint8Array<ArrayBuffer>], { type: video.mediaType ?? 'video/mp4' });
  await put(pathname, body, { access: 'private', contentType: video.mediaType ?? 'video/mp4', addRandomSuffix: false, allowOverwrite: true });
  await mutateOrder(orderId, (fresh) => {
    const current = fresh.scenes[sceneNumber - 1];
    if (!current) return;
    current.videoPathname = pathname;
...
export async function runDirectPreview(orderId: string) {
  for (let sceneNumber = 1; sceneNumber <= previewSceneCount(); sceneNumber += 1) {
    await generateScene(orderId, sceneNumber);
  }
  const order = await readOrder(orderId);
  if (!order) throw new Error('Order not found');
  ...
  latest.previewMoviePathname = assets.moviePathname;
  latest.status = 'awaiting-payment';
