...
async function generateScene(orderId: string, sceneNumber: number, fulfillment = false) {
  ...
  if (scene.generation.attempts >= MAX_SCENE_ATTEMPTS) throw new Error(`CREDIT-GUARD scene ${sceneNumber} reached the ${MAX_SCENE_ATTEMPTS}-attempt generation limit; manual review required before spending another AI credit.`);
  let claimed = false;
  await mutateOrder(orderId, (fresh) => {
    ...
    if ((current.status === 'completed' && current.videoPathname) || current.status === 'submitted' || current.generation.attempts >= MAX_SCENE_ATTEMPTS) return;
  ...
  console.info('[DirectMovie] CREDIT-SPEND authorized exactly once for scene claim', { orderId, sceneNumber, fulfillment });
  try {
...
export async function runDirectFulfillment(orderId: string) {
  ...
  console.info('[DirectMovie] CREDIT-GUARD paid fulfillment budget', { orderId, missingPaidScenes: sceneNumbers.length, sceneNumbers });
