...
async function assembleVerifiedStoredPreview(orderId: string) {
  ...
  console.info('[DirectPreview] verified stored preview', { orderId, stored: checks.filter((x) => x.exists).length });
...
async function generateScene(orderId: string, sceneNumber: number, fulfillment = false) {
  ...
  if (scene.status === 'completed' && scene.videoPathname) { console.info('[DirectMovie] scene reused', { orderId, sceneNumber }); return; }
  if (await recoverStoredScene(orderId, sceneNumber)) return;
  if (scene.status === 'submitted') { console.info('[DirectMovie] scene already in flight; skipping duplicate', { orderId, sceneNumber }); return; }
  let claimed = false; await mutateOrder(orderId, (fresh) => { const current = fresh.scenes[sceneNumber - 1]; if (!current) return; if ((current.status === 'completed' && current.videoPathname) || current.status === 'submitted') return; current.status = 'submitted'; current.generation.attempts += 1; delete current.error; fresh.status = fulfillment ? 'fulfillment-in-progress' : 'preview-in-progress'; claimed = true; });
  if (!claimed) return;
  console.info('[DirectMovie] scene generation starting', { orderId, sceneNumber, fulfillment, attempt: scene.generation.attempts + 1 });
  try {
...
export async function runDirectFulfillment(orderId: string) {
  ...
  if (initial.purchase.status !== 'paid') throw new Error('Paid order required');
  if (initial.finalMoviePathname || initial.status === 'complete') return;
  const sceneNumbers = initial.scenes.filter((scene) => scene.number > 6).map((scene) => scene.number);
  ...
  console.info('[DirectMovie] paid fulfillment completed', { orderId, finalMoviePathname: assets.moviePathname });
}
