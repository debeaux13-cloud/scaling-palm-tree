...
export async function assembleMovie(orderId: string, clips: { number: number; pathname: string; narration: string }[], kind: 'preview' | 'final') {
  ...
  try {
    ...
    // All scene MP4s are generated by the same model/settings. Join their existing
    // encoded streams directly instead of re-encoding ~3 minutes on a 2-vCPU sandbox.
    // This still creates ONE physical continuous MP4; the 18 clips remain source assets only.
    console.info('[MovieAssembly] ffmpeg stream-copy concat starting', { orderId, kind });
    const concat = await sandbox.runCommand('bash', ['-lc', `ffmpeg -y -f concat -safe 0 -i ${shell(`${root}/concat.txt`)} -map 0 -c copy -movflags +faststart ${shell(`${root}/movie.mp4`)}`]);
    if (concat.exitCode !== 0) throw new Error(`[MovieAssembly] stream-copy concat failed: ${await concat.stderr()}`);
    console.info('[MovieAssembly] ffmpeg stream-copy concat complete', { orderId, kind });
