...
async function generateAndPersistScene(orderId: string, sceneNumber: number) {
  ...
  scene.generation = { ...scene.generation, attempts: scene.generation.attempts + 1 };
  order.status = sceneNumber <= previewSceneCount() ? 'preview-in-progress' : 'fulfillment-in-progress';
  ...
  // This workflow runs scenes serially. Persist the completed scene on the same
  // order snapshot owned by this durable step instead of doing a second
  // read-modify-write through an in-memory queue that cannot span function instances.
  scene.videoPathname = videoPathname;
...
async function requireCompletedScenes(orderId: string, count: number) {
  'use step';
  const order = await readOrder(orderId); if (!order) throw new Error('Order not found');
  const incomplete = order.scenes.slice(0, count).filter((scene) => scene.status !== 'completed' || !scene.videoPathname).map((scene) => scene.number);
  if (incomplete.length) throw new Error(`Generation returned before persisted completion for scenes: ${incomplete.join(', ')}`);
  return true;
}
...
async function generateBatch(orderId: string, sceneNumbers: number[]) { for (const number of sceneNumbers) await generateAndPersistScene(orderId, number); }
export async function movieWorkflow(orderId: string) {
  'use workflow';
  await generateBatch(orderId, [1, 2, 3, 4, 5, 6]); await requireCompletedScenes(orderId, 6); await assemble(orderId, 'preview');
  using payment = createWebhook(); await setPaymentWebhook(orderId, payment.url); await payment;
  const order = await planContinuation(orderId); const finalSceneNumbers = order.scenes.filter((scene) => scene.number > 6).map((scene) => scene.number);
  await generateBatch(orderId, finalSceneNumbers); await requireCompletedScenes(orderId, order.scenes.length); await assemble(orderId, 'final');
}
